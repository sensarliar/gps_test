...
void MainWindow::remoteDataIncoming_com4()
{
    ...
    //    char buff[2*(66+49+29)+30];
        static char buff_com4[2*(74+48+151+569)+30];
        char* buff_com4_cont_p = buff_com4;
        static int buff_com4_cont_len=0;

        int bytesRead_com4 = 0;


        if((buff_com4_cont_len>74+48+151+569)||(buff_com4_cont_len<0))
        {
            QMessageBox::warning(this, tr("Error"), tr("Receive error2!"));
            buff_com4_cont_len = 0;
            return;
        }
        buff_com4_cont_p=&buff_com4[buff_com4_cont_len];
        if(buff_com4_cont_p==NULL)
        {
            QMessageBox::warning(this, tr("Error"), tr("Receive error1!"));
            return;
        }
            bytesRead_com4=read(m_fd_com4, buff_com4_cont_p, 2*(74+48+151+569)-buff_com4_cont_len);
            if (bytesRead_com4<1) {
                QMessageBox::warning(this, tr("Error"), tr("Receive error3!"));
                return;
            }

        bytesRead_com4 += buff_com4_cont_len;
      ...
      //  gps_impl_init();
          gps_nmea2.msg_available = FALSE;
        int temp_count_i = 0;
        char* dest_p =buff_com4;
        char* src_p = buff_com4;

     while(count_i<bytesRead_com4)
    {
         // buff_com4_cont_p = &nmea_parse_char(buff[count_i];
         temp_count_i = count_i;

         src_p = &buff_com4[count_i];
    // ui->m_receiveEdit->append(QString("\nstart deal with ..."));
         while (count_i<bytesRead_com4&&!gps_nmea2.msg_available)
         { nmea_parse_char2(buff_com4[count_i]);
             count_i++;
         }
    // ui->m_receiveEdit->append(QString("\nread buff over ..."));
        if (gps_nmea2.msg_available) {
     ////////     nmea_parse_msg();

          gps_nmea2.msg_available = FALSE;
          if(gps2.bestvela_parse_ok)
          {
             // serial_wr_func();

              gps2.bestvela_parse_ok =0;
          }
        ...
        else
          {
            ...
            if(temp_count_i>0)
            {
                buff_com4_cont_len = bytesRead_com4 - temp_count_i;
            ...
            {
                buff_com4_cont_len = bytesRead_com4;
                gps_nmea2.msg_len = 0;
                return;
            }
            else
